...
def normalize_address(address):
    ...
    for pattern in country_replacements:
        address = re.sub(pattern, country_replacements[pattern], address, flags=re.IGNORECASE)
...
def autocomplete_check(donee_info_gp):
    gp_address = donee_info_gp["address"] + ", " + donee_info_gp["city"] + ", " + donee_info_gp["state"] + ", " + donee_info_gp["country"]
    ...
    if len(autocomplete_results) > 0:
        for suggestion in autocomplete_results.get("suggestions", []):
            autocomplete_address = suggestion.get("placePrediction", {}).get("structuredFormat", {}).get("secondaryText", {}).get("text", "")
            if autocomplete_address:
                similarity_score = fuzzy_address_check(autocomplete_address, gp_address)
                print(f"auto address: {autocomplete_address}, donee_info address: {gp_address}, sim_score: {similarity_score}")
                if similarity_score > 80:
                    return True, suggestion.get("placePrediction", {}).get("placeId", "")
    return False, ""
